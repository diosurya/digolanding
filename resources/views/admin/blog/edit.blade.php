            ...
            <div class="lg:col-span-2 space-y-8">
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                    <div class="mb-6">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $post->title) }}" placeholder="Masukkan judul yang menarik..." class="w-full px-6 py-4 rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-slate-900" required>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative">
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $post->slug) }}" placeholder="slug-artikel-anda" class="w-full px-6 py-4 rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-slate-900 pr-12">
                        ...
                        <p id="slug-preview" class="mt-2 text-[10px] text-slate-400 font-medium italic">Preview: {{ url('/blog/') }}/<span id="slug-text">{{ $post->slug }}</span></p>
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $post->meta_title) }}" class="w-full px-6 py-4 rounded-2xl border-slate-800 bg-slate-800/50 focus:bg-slate-800 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-bold text-white placeholder-slate-600">
                            ...
                            <div>
                                ...
                                <textarea name="meta_description" id="meta_description" rows="3" class="w-full px-6 py-4 rounded-2xl border-slate-800 bg-slate-800/50 focus:bg-slate-800 focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-bold text-white placeholder-slate-600">{{ old('meta_description', $post->meta_description) }}</textarea>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                    ...
                    <div class="mb-8">
                        <label class="flex items-center gap-3 cursor-pointer group">
                            ...
                            <span class="text-sm font-bold text-slate-600 group-hover:text-slate-900 transition-colors">Publish Artikel</span>
                        </label>
                    </div>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                    ...
                    <div class="relative group cursor-pointer" onclick="document.getElementById('featured_image').click()">
                        <div class="w-full aspect-video rounded-2xl bg-slate-50 border-2 border-dashed border-slate-100 flex flex-col items-center justify-center p-4 group-hover:bg-slate-100/50 group-hover:border-indigo-200 transition-all overflow-hidden">
                            <img id="preview-image" src="{{ $post->featured_image ? asset('storage/' . $post->featured_image) : '' }}" class="{{ $post->featured_image ? '' : 'hidden' }} w-full h-full object-cover">
                            <div id="upload-placeholder" class="{{ $post->featured_image ? 'hidden' : '' }} text-center">
                                <i data-lucide="cloud-upload" class="w-8 h-8 text-slate-300 mx-auto mb-2 group-hover:text-indigo-400 transition-all"></i>
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Klik untuk Ubah</p>
                            </div>
                        </div>
...
<script>
    ...
    function checkSlugAvailability(slug) {
        ...
        slugTimeout = setTimeout(() => {
            fetch(`{{ route('admin.blog.checkSlug') }}?slug=${slug}&id={{ $post->id }}`)
